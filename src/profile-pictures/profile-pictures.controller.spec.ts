...
describe("ProfilePicturesController", () => {
  ...
  beforeEach(async () => {
      ...
      providers: [
        {
          ...
          useValue: {
            ...
            update: jest.fn((id: string, body: UpdateProfilePictureDto) => {
              const item = InMemoryProfilePictures.find(
                (item) => item.id === id
              );
            ...
            create: jest.fn((body: CreateProfilePictureDto) => {
              ...
              return InMemoryProfilePictures[
                InMemoryProfilePictures.length - 1
              ];
            }),
            remove: jest.fn(),
          },
    ...
    controller = module.get<ProfilePicturesController>(
  ...
  describe("create", () => {
    it("should create a profile picture successfully", async () => {
      ...
      expect(result).toEqual(
        InMemoryProfilePictures[InMemoryProfilePictures.length - 1]
      );
  ...
  describe("delete", () => {
    ...
    it("should throw an error if the profile picture to remove does not exist", async () => {
      ...
      jest
        .spyOn(service, "remove")
        .mockRejectedValueOnce(new Error("profile picture not found"));

      await expect(
        controller.remove(nonExistingProfilePictureId)
